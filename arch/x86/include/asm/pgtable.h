...
static inline int native_pmd_bad(pmd_t pmd)
{
    ...
    return (pmd_flags(pmd) & ~(_PAGE_USER | NEXT_LEVEL_FOLDED_MASK)) != _KERNPG_TABLE;
#else
...
#elif defined (CONFIG_X86_64_FPT)
    return (pud_flags(pud) & ~(_KERNPG_TABLE | _PAGE_USER | NEXT_LEVEL_FOLDED_MASK)) != 0;
#else
...
static inline int native_p4d_bad(p4d_t p4d)
{
    ...
    ignore_flags |= NEXT_LEVEL_FOLDED_MASK;
#endif
...
#else
	if (mm != &init_mm) {
		WARN(1, "set_pte_at called with non-init_mm mm addr=%lx ptep at %llx\n", addr, (uint64_t) ptep);
	}
