...
static void __init reset_early_hpt(void)
{
	...
	/* turn on user/kernel speration */
	/* Note: change page offset when switching to 5 level paging */
	wrmsrl(MSR_KERNEL_START, PAGE_OFFSET);
...
unsigned long early_pf_addrs[EARLY_PF_MAX] = {};
unsigned int early_pf_count = 0;
unsigned int early_pf_invalidated = 0;
...
static bool __init early_make_hpt(unsigned long address)
{
	...
	early_pf_addrs[early_pf_count++] = address;
	found = check_ecpt_all_way_loaded(ecpt_desc_ptr);
