...
void set_pte_vaddr(unsigned long vaddr, pte_t pteval)
{
	...
	/* dirty fix. for some reason this set_pte_vaddr always appears in dmesg */
...
static void invalidate_early_pf_direct_mapping(void)
{
	unsigned int i = early_pf_invalidated;
	for (; i < early_pf_count; i++) {
		/* Things from early_pf_handler which calls on early_top_pgt
			but it shouldn't come in with init_top_pgt
		 */
		if (early_pf_addrs[i] < __START_KERNEL_map && early_pf_addrs[i] >= PAGE_OFFSET) {
			early_ecpt_invalidate(init_mm.map_desc, early_pf_addrs[i]);
		}
		early_pf_invalidated++;
	}
}
			       ...
			       pgprot_t prot, bool init)
{
	...
	invalidate_early_pf_direct_mapping();
	...
	pr_info("__kernel_physical_mapping_init: vaddr=%lx vaddr_end=%lx paddr=%lx paddr_end=%lx\n", vaddr,
		vaddr_end, paddr_start, paddr_end);
		...
		unsigned long end, int node, struct vmem_altmap *altmap)
{
	...
	pr_info_verbose("%s: start: %lx end: %lx node: %x altmap: %lx\n",
		 	__func__,
			start,
			end,
			node,
