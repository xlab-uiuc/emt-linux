...
#if defined (CONFIG_X86_64_ECPT) || defined (CONFIG_X86_64_FPT)
...
int pud_set_huge(pud_t *pud, phys_addr_t addr, pgprot_t prot)
{
	...
	/**
	 * Practically, the ECPT version code can handle both.
	 * Let's keep the original version for now.
	 */
#ifdef CONFIG_X86_64_ECPT
	pte = pfn_pte(
		(u64)addr >> PAGE_SHIFT,
	...
	set_pud_at(&init_mm, addr, pud, pudval);
#else
	/* original code */
...
int pmd_set_huge(pmd_t *pmd, phys_addr_t addr, pgprot_t prot)
{
	...
	set_pmd_at(&init_mm, addr, pmd, pmdval);
#else
	/* original code */
