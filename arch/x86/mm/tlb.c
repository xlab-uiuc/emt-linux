...
static void load_new_mm_cr3_fpt(struct fpt_desc *fpt_desc, u16 new_asid, bool need_flush)
{
	unsigned long new_mm_cr3;

	if (need_flush) {
		invalidate_user_asid(new_asid);
		new_mm_cr3 = build_cr3(fpt_desc->pgd, new_asid);
	} else {
		new_mm_cr3 = build_cr3_noflush(fpt_desc->pgd, new_asid);
	}

	if (fpt_desc->pgd_folded) {
		new_mm_cr3 |= CR3_L4_L3_FOLDED_BIT;
		pr_info_verbose("FPT flush cr3=%lx\n", new_mm_cr3);
	}
			...
			struct task_struct *tsk)
{
	...
	if (need_flush) {
		...
		#elif defined (CONFIG_X86_64_FPT)
			if (next == &init_mm)
				/* we don't support FPT for kernel mappings now */
				load_new_mm_cr3(next->pgd, new_asid, true);
			else
				load_new_mm_cr3_fpt(next->map_desc, new_asid, true);
		#else
			load_new_mm_cr3(next->pgd, new_asid, true);
		#endif		
		...
		if (next == &init_mm)
			...
			load_new_mm_cr3_fpt(next->map_desc, new_asid, false);
		#else
			load_new_mm_cr3(next->pgd, new_asid, false);
		#endif
