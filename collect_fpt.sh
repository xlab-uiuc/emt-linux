...
do
    ...
    echo "run_linux_free_cmd --arch $ARCH --thp $thp_config --cmd ${COMMAND} --out ${OUTPUT_DIR}/${FILE_PREFIX}_walk_log.bin --image $image_path"
    # do_task $bench &
    if [ "$dry_run" = "false" ]; then
        ./run_linux_free_cmd --arch $ARCH --thp $thp_config --cmd "${COMMAND}" --out ${OUTPUT_DIR}/${FILE_PREFIX}_walk_log.bin --image $image_path &
    fi
