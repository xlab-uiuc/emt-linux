...
OUTPUT_DIR="/data2/fpt_kexec/${ARCH}_${FLAVOR}/"
...
OUTPUT_DIR=${OUTPUT_DIR}/${stage_str}
mkdir -p $OUTPUT_DIR

# while ps -p 1714721 > /dev/null; do sleep 30 ;echo "wait 30" ; done
N_REP=1
for bench in "${BENCHS[@]}"
do
    ...
    FILE_PREFIX="${ARCH}_${FLAVOR}_${thp_config}_${bench}_${stage_str}"
    ...
    echo "run_linux_free_cmd --arch $ARCH --thp $thp_config --cmd ${COMMAND} --out ${OUTPUT_DIR}/${FILE_PREFIX}_walk_log.bin --image $image_path --k_exec_only"
    # do_task $bench &
    if [ "$dry_run" = "false" ]; then
        ./run_linux_free_cmd --arch $ARCH --thp $thp_config --cmd "${COMMAND}" --out ${OUTPUT_DIR}/${FILE_PREFIX}_walk_log.bin --image $image_path --k_exec_only &
    fi
