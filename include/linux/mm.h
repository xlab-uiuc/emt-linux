...
/* #ifdef CONFIG_ENHANCED_INTERFACE
	if (!(__FILE__[0] == 'a' && __FILE__[1] == 'r' && __FILE__[2] == 'c')) { \
		WARN(1, "%s not implemented with get_locked_pte\n", __func__);	\
	}
#endif */
...
static inline pte_t * get_locked_tentry_pte(struct mm_struct *mm, unsigned long addr,
				    tentry_t *tentry, spinlock_t **ptl)
{
	pte_t *ptep;
	pgtable_level level = PG_LEVEL_PTE;
	read_tentry(mm, addr, tentry, &level);
	pr_tentry_verbose(tentry);
	// UNUSED(map_err);
	*ptl = aspace_lockptr(mm, tentry, addr, PG_LEVEL_PTE);
	spin_lock(*ptl);
...
#define pte_alloc_kernel(pmd, address)			\
	((unlikely(gen_pte_void(pmd, PG_LEVEL_PMD, &init_mm, address, 0)) \
