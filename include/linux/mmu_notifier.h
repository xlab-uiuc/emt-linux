...
#define clear_tentry_ptep_clear_flush_notify(__vma, __address, __tentry_p) \
({									\
	unsigned long ___addr = __address & PAGE_MASK;			\
	struct mm_struct *___mm = (__vma)->vm_mm;			\
	pte_t ___pte;							\
									\
	/* ___pte = ptep_clear_flush(__vma, __address, __ptep); */ \
	___pte = tentry_ptep_clear_flush(__vma, __address, __tentry_p);		\
	mmu_notifier_invalidate_range(___mm, ___addr,			\
					___addr + PAGE_SIZE);		\
									\
	___pte;								\
...
#define insert_tentry_pte_notify(__mm, __address, __tentry_p, __pte)		\
({									\
	struct mm_struct *___mm = __mm;					\
	unsigned long ___address = __address;				\
	pte_t ___pte = __pte;						\
	gen_pte_t gen_pte = { .pte = ___pte };				\
									\
	mmu_notifier_change_pte(___mm, ___address, ___pte);		\
	insert_tentry(__mm, __address, __tentry_p, PG_LEVEL_PTE, gen_pte);			\
})
