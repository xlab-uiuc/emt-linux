...
({									\
	...
	if (!(__func__[0] == 'f' && __func__[1] == 'p' && __func__[2] == 't') && mm != &init_mm) { /* hack */\
		WARN(1, "%s redirected to old routine\n", __func__); \
	} \
...
static inline pte_t *virt_to_kpte(unsigned long vaddr)
{
	...
	return gen_pte_void(gen_pte, PG_LEVEL_PMD, &init_mm, vaddr, 0) ?
		       NULL :
...
#ifdef CONFIG_PGTABLE_OP_GENERALIZABLE
#define pgd_offset_gate(mm, addr)	pgd_offset_map_with_mm(mm, addr)
#else
 ...
 */
 ...
 /* The function is used to implement pmd_next_level_not_accessible 
	when CONFIG_PGTABLE_OP_GENERALIZABLE is defined.
  */
...
static inline int pmd_trans_unstable_aspace(pmd_t *pmd, struct mm_struct *mm,
				     unsigned long addr)
{
	...
	if (pmd == NULL)
		return 0;
#ifdef CONFIG_TRANSPARENT_HUGEPAGE
	return pmd_next_level_not_accessible_trans_huge(pmd, mm, addr, 1);
#else
	return 0;
#endif
}

static inline int pmd_devmap_trans_unstable_aspace(pmd_t *pmd, struct mm_struct *mm,
				     unsigned long addr)
{
	return (pmd != NULL) && (pmd_devmap(*pmd) || pmd_trans_unstable_aspace(pmd, mm, addr));
}
