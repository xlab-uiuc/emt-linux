...
static struct page *follow_page_pte(struct vm_area_struct *vma,
		unsigned long address, tentry_t *tentry, unsigned int flags,
		struct dev_pagemap **pgmap)
{
...
retry:
	...
	ptl = aspace_lockptr(mm, tentry, address, PG_LEVEL_PTE);
	spin_lock(ptl);
	ptep = GENPTEP_TO_PTEP(tentry->ptep[PG_LEVEL_PTE]);
	...
	if (!pte_present(pte)) {
		...
		migration_entry_wait_enhanced(mm, tentry, address);
		goto retry;
	}
...
static struct page *follow_pmd_mask(struct vm_area_struct *vma,
				    unsigned long address, tentry_t *tentry,
				    unsigned int flags,
				    struct follow_page_context *ctx)
{
	...
	if (gen_pte_void(gen_pte, PG_LEVEL_PMD, mm, address, 1))
		return no_page_table(vma, flags);
	/* original code */
	// if (pmd_none(pmdval))
		// return no_page_table(vma, flags);
...
retry_locked:
	...
	gen_pte.pmd = *pmd;
	if (unlikely(gen_pte_void(gen_pte, PG_LEVEL_PMD, mm, address, 1))) {
		spin_unlock(ptl);
		return no_page_table(vma, flags);
	}
	/* original code */
	...
	if (flags & FOLL_SPLIT_PMD) {
		...
		if (is_huge_zero_page(page)) {
			...
			if (pmd_trans_unstable_aspace(pmd , vma->vm_mm, address))
				return 0;
			      ...
			      struct follow_page_context *ctx)
{
	...
	int map_err = 0;
	tentry_t tentry = {0};
	pgtable_level pg_size = 0;

	ctx->page_mask = 0;
	
	map_err = read_tentry(mm, address, &tentry, &pg_size);
	
	pr_info_verbose("map_err=%d addr=%lx pg_size=%d\n", map_err, address, pg_size);
	...
	if (map_err) {	
		...
		if (pg_size == PG_LEVEL_PTE) {
			page = follow_page_pte(vma, address, &tentry, flags, &ctx->pgmap);
		} else if (pg_size == PG_LEVEL_PMD) {
			page = follow_pmd_mask(vma, address, &tentry, flags, ctx);
		} else {
			/* not implemented */
			WARN(1, "pg_size =%d\n", pg_size);
			page = ERR_PTR(-EINVAL);
		...
		struct page **page)
{
	...
	if (address > TASK_SIZE){
		pgd = pgd_offset_map_with_mm(&init_mm, address);
		// pgd = pgd_offset_k(address);
	} else {
		pgd = pgd_offset_gate(mm, address);
	}
	
	if (no_p4d_and_lower_pgtable(*pgd)) {
		return -EFAULT;
	}
	// if (pgd_none(*pgd))
	// 	return -EFAULT;
	...
	if (no_pud_and_lower_pgtable(*p4d))
		return -EFAULT;
	// if (p4d_none(*p4d))
	// 	return -EFAULT;

	pud = pud_offset_map_with_mm(mm, p4d, address);
	if (no_pmd_and_lower_pgtable(*pud))
		return -EFAULT;
	// if (pud_none(*pud))
		// return -EFAULT;
		...
		struct vm_area_struct **vmas, int *locked)
{
	...
	do {
		...
		if (!vma || start >= vma->vm_end) {
			...
			pr_info_verbose("in_gate_area=%d start=%lx VSYSCALL_ADDR=%lx\n",
			 	in_gate_area(mm, start), start, VSYSCALL_ADDR);
...
#ifdef CONFIG_ENHANCED_INTERFACE
static int gup_pte(tentry_t * tentry, unsigned long addr,
	unsigned int flags, struct page **pages, int *nr)
{
	struct dev_pagemap *pgmap = NULL;
	int nr_start = *nr, ret = 0;
	pte_t * ptep = GENPTEP_TO_PTEP(tentry->ptep[PG_LEVEL_PTE]);
	pte_t *ptem = ptep;
	...
	*/
	...
	pages[*nr] = page;
	(*nr)++;
	pr_info_verbose("gup_pte: addr=%lx pte=%lx pages[%d] pfn=%lx\n", 
		addr, pte.pte, *nr - 1, page_to_pfn(page));
pte_unmap:
	if (pgmap)
			 ...
			 unsigned int flags, struct page **pages, int *nr)
{
	...
	ptem = ptep = pte_offset_map_with_mm(current->mm, &pmd, addr);
	...
	do {
		...
		&& (ptep = ptep_get_next(current->mm, ptep, addr - PAGE_SIZE)));
...
{
	tentry_iter_t iter = {};
	tentry_iter_init(current->mm, addr, end, &iter);

	do {
		if (!iter.maperr) {
			if (iter.pg_level == PG_LEVEL_PTE) {
				pages += gup_pte(&iter.tentry, iter.addr, flags, pages, nr);
			} else {
		...
		unsigned int flags, struct page **pages, int *nr)
{
	...
	pmdp = pmd_offset_map_with_mm(current->mm, pudp, addr);
	/* original code */
	...
	do {
		...
		if (gen_pte_void(gen_pte, PG_LEVEL_PMD, current->mm, addr, 1))
			return 0;
		// if (!pmd_present(pmd))
		// 	return 0;
		...
		(pmdp = pmdp_get_next(current->mm, pmdp, addr), addr = next) : 
		(addr = next, 0));
			 ...
			 unsigned int flags, struct page **pages, int *nr)
{
	...
	pudp = pud_offset_map_with_mm(current->mm, p4dp, addr);
	/* original code */
	...
	do {
		...
		if (unlikely(pud_huge(pud))) {
			...
			(pudp = pudp_get_next(current->mm, pudp, addr), addr = next) : 
			(addr = next, 0));
			 ...
			 unsigned int flags, struct page **pages, int *nr)
{
	unsigned long next;
	p4d_t *p4dp;
	p4dp = p4d_offset_map_with_mm(current->mm, pgdp, addr);
	/* original code */
	...
	do {
		...
		if (unlikely(is_hugepd(__hugepd(p4d_val(p4d))))) {
			...
			(p4dp = p4dp_get_next(current->mm, p4dp, addr), addr = next) : 
			(addr = next, 0));
		...
		unsigned int flags, struct page **pages, int *nr)
{
	...
	pgdp = pgd_offset_map_with_mm(current->mm, addr);
	/* original code */
	...
	do {
		...
		(pgdp = pgdp_get_next(current->mm, pgdp, addr), addr = next) : 
		(addr = next, 0));
