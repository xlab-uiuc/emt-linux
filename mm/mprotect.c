...
static unsigned long change_pte_prot(struct vm_area_struct *vma, tentry_t * tentry,
		...
		unsigned long cp_flags)
{
	...
	pr_info_verbose("addr=%lx newprot=%lx\n", 
		addr, newprot.pgprot);
	 ...
	 */
	// if (pmd_trans_unstable_aspace(pmd , vma->vm_mm, addr))
	// 	return 0;

	if (aspace_trans_unstable(vma->vm_mm, tentry, addr, PG_LEVEL_PMD)) {
		pr_info_verbose("return from aspace_trans_unstable\n");
		return 0;
	}
	/* original code */
	...
	/* NOTE: this can be better optimized */
	...
	if (pte_present(oldpte)) {
		...
		gen_pte.pte = ptent;
		update_tentry(vma->vm_mm, addr, tentry, PG_LEVEL_PTE, gen_pte, 0);
		...
		unsigned long cp_flags)
{
	...
	pr_info_verbose("vma at %llx mm at %llx pgd at %llx map_desc at %llx start=%lx end=%lx newprot=%lx\n", 
		(uint64_t) vma, (uint64_t) mm, (uint64_t) mm->pgd, 
	...
	// read_tentry(vma->vm_mm, addr, &tentry, &pg_level);	
	tentry_iter_init(vma->vm_mm, addr, end, &iter);
	
	// for (;tentry_iter_has_next(&iter); tentry_iter_next(&iter)) {
	do {
		if (!iter.maperr) {
			if (iter.pg_level == PG_LEVEL_PTE) {
				pages += change_pte_prot(vma, &iter.tentry, iter.addr, newprot, cp_flags);
			} else {
 ...
 */
static inline int pmd_none_or_clear_bad_unless_trans_huge(struct mm_struct * mm, pmd_t *pmd, unsigned long address)
{
	pmd_t pmdval = pmd_read_atomic(pmd);
	gen_pte_t gen_pte = { .pmd = pmdval };
		...
		pgprot_t newprot, unsigned long cp_flags)
{
	...
	do {
		...
		if (!is_swap_pmd(*pmd) && !pmd_devmap(*pmd) &&
		     pmd_none_or_clear_bad_unless_trans_huge(vma->vm_mm, pmd, addr))
			goto next;
