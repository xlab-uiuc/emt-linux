...
static __maybe_unused pmd_t *get_old_pmd(struct mm_struct *mm, unsigned long addr)
{
	...
	gen_pte.pmd = *pmd;
	/* get_old_pmd won't build overlapping PMD on top of PTEs */
	/* It's safe to skip the check and assume PTE exits */
	if (gen_pte_void(gen_pte, PG_LEVEL_PMD, mm, addr, 1))
		return NULL;
#else
...
static void move_tentries(struct vm_area_struct *vma, tentry_t * old_tentry,
	unsigned long old_addr,
	struct vm_area_struct *new_vma, tentry_t * new_tentry,
	unsigned long new_addr, bool need_rmap_locks) 
{
	...
	pr_info_verbose("old_addr=%lx new_addr=%lx\n", old_addr, new_addr);
	...
	old_ptl = aspace_lockptr(mm, old_tentry, old_addr, PG_LEVEL_PTE);
	spin_lock(old_ptl);
	new_ptl = aspace_lockptr(new_vma->vm_mm, new_tentry, new_addr, PG_LEVEL_PTE);
	...
	// for (; old_addr < old_end; old_pte = ptep_get_next(mm, old_pte, old_addr),
	// 				old_addr += PAGE_SIZE,
	// 			   new_pte = ptep_get_next(new_vma->vm_mm, new_pte, new_addr),
	// 			    	new_addr += PAGE_SIZE) {
	old_pte = GENPTEP_TO_PTEP(old_tentry->ptep[PG_LEVEL_PTE]);
		if (pte_none(*old_pte))
			goto DONE;

		pte = clear_tentry(mm, old_addr, old_tentry, PG_LEVEL_PTE).pte;
		...
		gen_pte.pte = pte;
		ins_res = insert_tentry(new_vma->vm_mm, new_addr, new_tentry, PG_LEVEL_PTE, gen_pte);
		if (ins_res) {
...
DONE:
	...
	if (force_flush)
		flush_tlb_range(vma, old_addr, old_addr + PAGE_SIZE);
	if (new_ptl != old_ptl)
		spin_unlock(new_ptl);
		  ...
		  unsigned long new_addr, pmd_t *old_pmd, pmd_t *new_pmd)
{
	...
	WARN(1, "move_normal_pmd not supported with generalized interface!\n");
	...
	bool need_rmap_locks)
{
	...
	tentry_iter_init(vma->vm_mm, old_addr, old_addr + len, &src_iter);
	tentry_iter_init(new_vma->vm_mm, new_addr, new_addr + len, &dst_iter);


	pr_info_verbose("move_page_tables old_addr=%lx new_addr=%lx len=%lx\n", old_addr, new_addr, len);
	...
	do {
		if (!src_iter.maperr) {
			if (src_iter.pg_level == PG_LEVEL_PTE) {
				move_tentries(vma, &src_iter.tentry, src_iter.addr,
					new_vma, &dst_iter.tentry, dst_iter.addr,
					need_rmap_locks);
			} else {
	...
	// #ifdef CONFIG_PGTABLE_OP_GENERALIZABLE
	// 			if (pmd_trans_unstable_aspace(old_pmd , vma->vm_mm, old_addr))
	// 				return 0;
	// #else
