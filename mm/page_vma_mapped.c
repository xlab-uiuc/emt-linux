...
bool page_vma_mapped_walk(struct page_vma_mapped_walk *pvmw)
{
	...
	pr_info_verbose("page_vma_mapped_walk: pfn=%lx address=%lx\n", page_to_pfn(page), pvmw->address);
...
restart:
	do {
		...
		pr_info_verbose("pgd offset=%lx end=%lx\n", pgd_val(*pgd), end);
		if (!pgd_present(*pgd)) {
			step_forward(pvmw, PGDIR_SIZE);
			pr_info_verbose("pvmw->address=%lx end=%lx\n", pvmw->address, end);
			continue;
		}
...
#ifdef CONFIG_PGTABLE_OP_GENERALIZABLE
			pr_info("pvmw->pte at %llx =%llx\n", (uint64_t) pvmw->pte, (uint64_t) pvmw->pte->pte);
			pvmw->pte = ptep_get_next(pvmw->vma->vm_mm, pvmw->pte, pvmw->address);
#else
			pvmw->pte++;
#endif
