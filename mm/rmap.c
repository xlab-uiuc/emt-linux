...
pmd_t *mm_find_pmd(struct mm_struct *mm, unsigned long address)
{
	...
	/* function contains barrier check */
	/* If it is trans huge, we set pmd=NULL to indicate that there's no associated PTEs */
	/* If it is next level doesn't contain valid ptes (gen_pte_void), we also set pmd=NULL */
	/* We pass built_hint = 1, to not enforce checking  */
	if (pmd_next_level_not_accessible_trans_huge(pmd, mm, address, 1))
		pmd = NULL;
