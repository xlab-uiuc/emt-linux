			     ...
			     pgtbl_mod_mask *mask)
{
	...
	do {
		...
		cleared = pmd_clear_huge_with_addr(pmd, addr);
		...
		if (pmd_next_level_not_accessible_regular(pmd, &init_mm, addr,
							  1))
			continue;
			     ...
			     pgtbl_mod_mask *mask)
{
	...
	do {
		...
		cleared = pud_clear_huge_with_addr(pud, addr);
			     ...
			     pgtbl_mod_mask *mask)
{
	...
	do {
		...
		cleared = p4d_clear_huge_with_addr(p4d, addr);
...
struct page *vmalloc_to_page(const void *vmalloc_addr)
{
	...
	pmd = pmd_offset_map_with_mm(&init_mm, pud, addr);
	gen_pte.pmd = *pmd;
	/* we can skip the partial built check and lookup pte later */
	if (gen_pte_void(gen_pte, PG_LEVEL_PMD, &init_mm, addr, 1))
		return NULL;
#else
	pmd = pmd_offset(pud, addr);
	if (pmd_none(*pmd))
